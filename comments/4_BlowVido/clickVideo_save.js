...
function processFrame(){
	...
	if(!isNaN(video.duration)){
		if(SOURCERECT.width==0)
		{
			...
			//初始化小方块
			createTiles();			/*console.log(SOURCERECT);*/
		}
	}
	//绘制拷贝用的copy canvas画布
	...
	for (var i = 0; i <tiles.length; i++) {
		...
		if(tile.force>0.0001){
			...
			if(tile.currentY<=0||tile.currentX>=PAINTRECT.height){
				tile.moveY *=-1;
			}
...
function processFrame(){
	...
	if(!isNaN(video.duration)){
		...
		///保持循环播放，也可直接在vido属性里面添加循环属性
		/*if(video.currentTime==video.duration){
			video.currentTime=0;//当当前时间为视屏的是假 长度，也就是播放到结束
		}*/
	}
	//var debugStr="";
